...
# 1. 定義要標註的「真名名單」 (已更新)
# ==========================================
# 程式會自動搜尋呢個 List 入面嘅字，標記做 B-NAME / I-NAME
# 其他未提及嘅詞語 (如 爹/娘/男人) 會自動變成 O
target_names = [
    # 主角一家
    "福貴", "家珍", "鳳霞", "有慶", 
    # 親戚與家族姓氏
...
def process_novel(text):
    # A. 清洗
    garbage_pattern = r"\*沂怯滯純煊紙粽牛乇鶚悄歉黿\*張"
    corrected_text = "我是又痛快又緊張，特別是那個緊張"
    text = re.sub(garbage_pattern, corrected_text, text)
    text = text.replace("|", "")  # 移除防屏蔽符號
    
    # B. 簡單分句 (按句號/問號/感嘆號/換行 切分)
    ...
    # C. 自動標註
    # 這裡的 ID 必須同你 train_lora.py 入面定義嘅一樣
    label2id = {
...
print("👉 記得將 '徐家', '陳老闆', '長根', '沈先生' 等加入標記名單，模型先識得捉佢哋！")